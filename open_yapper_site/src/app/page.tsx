...
/* ========== DEMO STRINGS ========== */
const demoRaw =
  "Um so like I was thinking we could maybe send that email to the team today if that's cool with you I mean unless you have other stuff to do or something";
const demoRefined =
  "Let's send that email to the team today—unless you have other priorities.";
...
export default function OpenYapperPage() {
  return (
    ...
    >
      ...
      {/* ========== 2) HERO SECTION ========== */}
      <section id="features" className="relative flex min-h-[80vh] flex-col items-center justify-center overflow-hidden px-6 py-24 md:px-12 md:py-32">
        {/* Blurred lime blob */}
        <div
      ...
      {/* ========== 3) MARQUEE STRIP ========== */}
      <section className="overflow-hidden border-y-4 border-[#D4FF00] bg-[#0A0A0A] py-4" aria-hidden>
      ...
      <footer className="bg-[#0A0A0A] px-6 py-20 md:px-12">
        <div className="mx-auto flex max-w-6xl flex-col items-center gap-12 md:flex-row md:justify-between md:items-start">
          ...
          <div className="flex flex-col items-center gap-4 md:flex-row md:items-center md:gap-12">
            <p className="text-sm text-gray-400">© 2025 Open Yapper. All rights reserved.</p>
            <div className="flex gap-6">
              <a href="#" className="text-sm font-medium text-gray-400 transition-colors hover:text-[#D4FF00]">
                Twitter
              </a>
              <a href="#" className="text-sm font-medium text-gray-400 transition-colors hover:text-[#D4FF00]">
                Instagram
...
/* ========== DEMO SECTION (CLIENT INTERACTIVE) ========== */
function DemoSection() {
  const [isRecording, setIsRecording] = useState(false);
  const [demoStage, setDemoStage] = useState<"idle" | "recording" | "refining" | "done">("idle");
  ...
  const startRecording = useCallback(() => {
    ...
    intervalRef.current = setInterval(() => {
      if (wordIndexRef.current < words.length) {
        setRawText((prev) => prev + (prev ? " " : "") + words[wordIndexRef.current]);
      ...
      <div className="mx-auto grid max-w-6xl gap-16 lg:grid-cols-2 lg:gap-20">
        ...
        <div className="flex flex-col justify-center">
          ...
          <p className="mt-6 text-lg font-medium text-[#0A0A0A]/90 md:text-xl">
            Record your thoughts naturally. Our AI transcribes, removes filler words, and polishes your text. Copy and paste anywhere.
          </p>
        ...
        >
          ...
          <div className="flex justify-center border-t-2 border-[#0A0A0A] bg-[#F4F4F0] p-6">
            ...
            {demoStage === "recording" && (
              ...
              >
                <Square className="h-6 w-6 fill-[#0A0A0A] text-[#0A0A0A]" aria-hidden />
              </motion.button>
            )}
            {(demoStage === "refining" || demoStage === "done") && (
              <button
                ...
                aria-label={demoStage === "refining" ? "Processing" : "Start over"}
              >
                <Mic className="h-8 w-8 text-[#0A0A0A]" aria-hidden />
              </button>
            )}
